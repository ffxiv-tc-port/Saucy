...
/// <summary>
/// 持鎖時不要送聊天、不要寫 log、不要存設定：_preGameLock 由繪製端、framework 執行緒與預覽模擬的背景工作共用，這三件事各自會去搶別的元件的鎖或等磁碟（Serilog sink 自己有鎖也可能寫檔、EzConfig 存檔是整份序列化＋寫入），在鎖內做等於讓每個等鎖的人一起排隊。
/// 用 <see cref="Begin" /> 開一個延後範圍，範圍內要做的這些事先收下來，範圍結束（已經出鎖）之後再做完 —— 仍然在原本那個方法回傳之前同步完成，不是延後一幀，也不換執行緒。
/// 判斷「做不做」的條件全部留在原本的位置與時機，只有「做」這一步被移到鎖外。
/// 🔑 不在延後範圍內時每個入口都當場自己做（TryDefer 回 false）——所以漏包某一條持鎖呼叫鏈的後果是「維持原本的行為」，不會變成靜默不做。
/// </summary>
