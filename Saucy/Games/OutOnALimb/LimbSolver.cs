...
/// 【機制參考】判定的形狀參考了 DailyRoutines 的 <c>AutoOutOnALimb</c> 對外可見的行為與資料結構（它維護一個 <c>int[100]</c> 的逐位置嘗試表、並以四級結果 Fail／Normal／Great／Perfect 收斂），但**程式碼是我們自己寫的**：DR 未公開原始碼，我們只採用「怎麼做」的知識，沒有沿用它的實作。對應關係：DR 的四級結果＝我們的 <see cref="HitPower"/>（Addon#9710–9713 的四句手感文字），DR 的逐位置表＝這裡的 <see cref="board"/>。
/// 【與舊版的差別】盤面改成**整個 0–100 逐格**（跟 DR 的 <c>int[100]</c> 同解析度），設定裡的「步進值」不再是盤面格數，而是**粗掃時的最小間距**——一旦有手感就可以在比步進值更細的刻度上收斂。
/// </summary>
internal class LimbSolver
{
    ...
    /// 🔴 為什麼要內縮：**端點的資訊效率只有一半。** 最佳點的感應範圍是雙側的——探刻度 50 能感應到 50 兩側的最佳點，探刻度 100 卻只能感應到左半邊，右半邊在界外。
    /// ⚠️ 但**純二分（完全避開端點）會漏球**：最佳點落在極端位置時整局碰不到，正解是「保留最大間隙法，但把候選範圍內縮一個感應半徑」——端點區域改由**內側那一點的感應範圍**覆蓋，所以一個都不會漏。
    /// 📌 真實感應半徑是未知的遊戲常數（離線推不出來），所以內縮量取<c>coarseStep / 2</c>——粗掃間距本來就隱含「半徑約等於間距的一半」這個假設，兩者一致才自洽。再夾一個上限 5，免得使用者把步進值調很大時反而讓兩端一大片在刀數用完之前都掃不到（那才是真的回退）。
    ...
    /// 🔑 兩趟：**第一趟只收左右鄰居也沒試過的格子。** 因為斧頭實際落點會偏 ±1 格（見 <see cref="blocked"/> 的實機分布），挑一個緊貼著已知格的位置時，偏一格就直接落回那個已知格＝這一刀零收穫。要求鄰居也是空的，就算偏了一格也還是踩在沒問過的位置上，資訊不會白拿。
    /// 第二趟放掉這個要求（盤面後期本來就沒有孤立的空格了），行為與舊版相同，所以這只是**排序偏好**，不會讓原本找得到的候選變成找不到。</summary>
    ...
    /// <summary>粗掃：挑「離所有已試位置最遠」的那一格（最大間隙），而不是舊版的**隨機**挑。空盤面時最大間隙落在正中央，接著是兩端、再來是四等分點……等於自動做出一輪二分掃描，用同樣的刀數換到更均勻的覆蓋。</summary>
    /// <param name="minSpacing">候選點離最近取樣點至少要多遠。呼叫端先用設定的步進值撒開，撒不下去了再用 1 補洞——**絕不要在還有沒試過的格子時回 null 讓呼叫端去重砍舊位置**。</param>
    /// <param name="inset">候選範圍兩端各往內縮幾格（見 <see cref="CoarseInsetLimit"/>）。⚠️ 只縮**候選**範圍；算距離時仍然要看整個盤面上所有已試過的點，否則會把端點附近已經問過的資訊當成沒問過。</param>
