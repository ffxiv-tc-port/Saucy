...
/// 砍伐回饋                       → 系統訊息的四級手感（主）／樹的量表落差（補強）沒有封包 detour、沒有 hook、沒有特徵碼、沒有寫入遊戲記憶體，出手只有「在對的時機按下畫面上本來就有的按鈕」。
/// 【動作範圍】預設只在機台畫面已經開著時才動作。「連續遊玩」是**明確的選擇加上明確的一次按鈕**：設定要打開、面板上要按下開始、而且跑滿設定的局數就自己停。沒按開始就絕不會自己去碰機台。</summary>
public unsafe class OutOnALimbModule : Module
{
    ...
    /// 🔴 為什麼要有階梯：**單幀位移可以比目標格還寬**：這種情況下「某一幀剛好落在格子裡」是機率事件，不是必然，所以要允許放掉幾趟等下一趟。
    /// 🔴 但**絕不能等到卡死**：每一局都花掉一枚金碟幣，不停表等於錢丟掉。所以階梯的終點是「不管落在哪一格都按下去」——失敗形式是**打到隔壁格**，不是不揮斧。
    /// 0–<see cref="PreciseWindowMs"/>：只有落在目標格、而且離兩邊界都有餘裕才按。–<see cref="AnyInZoneMs"/>：落在目標格裡就按（不再要求餘裕）。
    /// –<see cref="WidenZoneMs"/>：放寬到「目標格或次寬的那一格」。之後：落在哪一格都按。
    /// ⚠️ 這幾個毫秒數是**取捨值不是實測常數**：訂太短會退化成舊行為（隨便按），訂太長則每局多站幾秒。實機 log 顯示一趟掃描約 200–300ms，所以 1500ms 大約是 5–7 趟。
    /// </summary>
    ...
    private void OnUpdate(IFramework framework)
    {
        try
        {
            ...
            // ・砍伐畫面開著時的確認框是「挑戰翻倍」，那由 HandleDoubleDownPrompt 依剩餘時間決定，不能讓連續遊玩搶答（它只會一律按「是」）。用畫面狀態把兩者結構性地分開，比只靠文字判斷可靠。
            // ・機台畫面還開著就去互動＝在遊戲進行中重新戳機台，所以互動路徑整條關掉，這個時機只允許「回答已經跳出來的確認框」。
    ...
    /// 🔴 力量表 addon 與礦脈探索共用，所以動它之前要先確認面前那台真的是孤樹無援；認不出來就完全不碰（玩家自己停表，砍伐階段照樣會接手）。
    /// 現在的出手條件改成連言：**當幀的指針必須真的落在目標格裡**（用遊戲自己的<c>pos &gt; low &amp;&amp; pos &lt;= high</c> 判定），再加上一個會隨時間放寬的精準度要求。**不需要、也不應該加「提前量」**。
    /// </summary>
    ...
    /// 所以不管伺服器把區間切成什麼樣子、也不管畫面上的區塊順序怎麼洗牌，索引 0（泰坦）永遠指最窄（最難停中）的那一格、索引 2（仙人掌怪）永遠指最寬的。</summary>
    ...
    /// <summary>
    /// 每一幀判斷「是不是換了一棵新的樹」。
    /// 🔴 **這裡刻意完全不看階段值。**一刀的循環是 <c>3 →(4)→ 5 → 7 → 3</c>——5 與 7 每一刀都會出現，於是**每一刀都被判成換樹**，解題器永遠回到初始狀態、目標從頭到尾固定在 20。
    /// 現在只採信兩個單調性訊號：剩餘揮擊次數**回升**、以及量表上限改變。兩者都不需要知道任何階段值的語意，改版動了階段機也不會壞。
    /// </summary>
    ...
    /// <summary>
    /// **補強**回饋：樹的量表在砍完之後掉了多少。
    /// 所以這條路徑保留，但只當額外訊號——它有資料就採信，沒資料也不影響收斂。結論沒有變（手感仍是主判據，因為盲掃階段砍不中就沒有傷害可量）。
    /// </summary>
    ...
    private void TrySwingAtTarget(AtkUnitBase* addon)
    {
        ...
        // 落到隔壁格本身可以接受（資訊差不多），但如果那個隔壁格**已經試過了**，這一刀就是零收穫。真正的對策在解題器那邊（挑候選時優先選左右鄰居也沒試過的格子，見 <see cref="LimbSolver"/> 的 NearestUntried）；這裡只負責**量**，不做決策。
        // 🔴 這裡刻意**不**去把打不到的目標標成「以後不要挑」。離線量測（tools/limbsim 閘門⑥與變體比較）顯示那樣做只會讓重複要求變少、命中率**反而略降**，在感應半徑最小（只有正中才算命中）的假設下降得最明顯——因為那等於把一個可能就是最佳點的格子丟掉。**症狀改善、結果變差的典型。**
    ...
    /// <summary>
    /// 連續遊玩：一局結束後自己跟機台開下一局。
    /// 🔴 三層閘門，每一層都必須成立才會動作：設定要打開、玩家要按過「開始連續遊玩」、而且局數還沒到上限。任何一層不成立就完全不碰機台。這裡不做任何「偵測到某某狀況就自動接手」的事——沒按開始就永遠不會有第一個動作。
    /// </summary>
    /// <param name="allowInteract">可不可以主動去戳機台。機台畫面還開著時一律 false——那個時機只准回答已經跳出來的確認框，不准發起新的互動。</param>
    ...
    /// <summary>
    /// 連續遊玩看到確認框時的判定。
    /// 🔴 這是整條路徑上**唯一會花掉金碟幣**的動作（每局 1 枚），所以判定寫成**連言**：下面每一條都成立才按「是」，任何一條不成立就完全不碰。寧可不按，也不要按錯。
    /// 確認框文字裡有**機台名稱**——執行期從 EObjName#2005423／Item#30425 讀，不寫死字串；
    /// 確認框文字命中 <see cref="LimbPrompt"/> 從 Addon#9321 拆出來的固定句。台服 1138 張表全掃證實「要挑戰一下嗎？」只有 9321 這一列有，而翻倍提示（9329／9333）寫的是「要<b>嘗試</b>挑戰一下嗎？」，兩者不會互相包含；
    /// 附近認得出孤樹無援機台（呼叫端已經要求，這裡不重複）；不是翻倍提示（<see cref="SelectYesnoHelper.LooksLikeArcadeDoubleDownPrompt"/>）；不是被封鎖的系統提示（傳送、捨棄、組隊邀請……）。
    /// </summary>
