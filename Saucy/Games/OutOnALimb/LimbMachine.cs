...
/// 為什麼需要：力量表畫面 <c>MiniGameAimg</c> 是孤樹無援與礦脈探索**共用**的，只憑它開著就自動停表，會連玩家在玩礦脈探索時也一起接手——那是玩家沒有啟用的東西。砍伐階段（<c>MiniGameBotanist</c>）本身就只屬於孤樹無援，不需要這層判斷。
/// 判斷方式刻意用**名稱**而不是 DataId：台服的 NPC／物件 DataId 與國際服不一定相同，名稱則是直接從當前用戶端的資料表讀出來的，一定跟畫面上顯示的一致。涵蓋兩種機台：金碟遊樂園的公共機台（EObjName#2005423）與房屋家具版（Item#30425）。
/// </summary>
internal static class LimbMachine
{
    ...
    /// 台服的街機遊玩確認框是 Addon 9321 的模板，第一行就是**粗體的機台名**（使用者回報的截圖第一行正是「孤樹無援」）。名稱一律從當前用戶端的資料表讀（<see cref="BuildMachineNames"/>：EObjName#2005423／Item#30425），**不寫死任何語言的字串**，所以換語言、改譯名都不會靜默失效。
