...
/// 【為什麼不寫死字串】repo 慣例是面向使用者的文字一律從 Lumina 讀，這裡也一樣：比對用的句子是**執行期**從 <c>Addon</c> 表第 <see cref="PlayConfirmAddonRowId"/> 列拆出來的，換語言、改譯名、甚至官方改寫句子都會自動跟上。寫死中文的失敗方式是靜默的。
/// 【為什麼這一列夠獨特】台服 7.20 的 <c>Addon</c> 表（1138 張表全掃）裡：「文字命中 9321 的固定句」等價於「這是遊玩確認框」，不會誤中砍完一棵樹之後那個免費的續戰提示。
/// 🔴 拆不出句子時一律回 false（＝不按）。這條路徑每按一次就花掉真的金碟幣，「認不出來就不要碰」永遠比「猜對機率很高」正確。
/// </summary>
