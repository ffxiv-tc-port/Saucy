...
public static unsafe class AddonButton
{
    /// <summary>
    /// 安全地讀取按鈕的啟用狀態。
    /// 🔴 <c>AtkComponentButton.IsEnabled</c> 的實作是<c>AtkComponentBase.OwnerNode-&gt;AtkResNode.NodeFlags.HasFlag(...)</c>，對 <c>OwnerNode</c> 沒有任何 null 檢查 —— <c>OwnerNode</c> 為 null 時會丟出AccessViolationException，而 AVE 在 .NET Core 是 corrupted-state exception，<c>try/catch</c> 攔不到。所有讀取 <c>IsEnabled</c> 的地方都必須改走這裡。
